...
static void pub_handler(const char *topic, uint16_t topic_len, const uint8_t *chunk, uint16_t chunk_len) {
	...
	if(strcmp((const char*) chunk, "INC") == 0) {
		LOG_INFO("INCREASING COOLANT FLOW\n");
		increase_coolant_flow = true;
	}
    else if(strcmp((const char*) chunk, "DEC") == 0) {
		LOG_INFO("DECREASING COOLANT FLOW\n");	
		increase_coolant_flow = false;
	}
}
...
PROCESS_THREAD(sensor, ev, data) {
	...
	while(true) {
        ...
        if(state == STATE_CONNECTED) {
            ...
            LOG_INFO("SUBSCRIBING TO coolant TOPIC\n");
        ...
        if(state == STATE_SUBSCRIBED) {
            if(increase_coolant_flow) variation = -(rand() % 3);
            else variation = (rand() % 3);
			pressure += variation

			LOG_INFO("NEW PRESSURE: %d\n", pressure);
			
			sprintf(pub_topic, "%s", "pressure");
			sprintf(app_buffer, "{\"value\": %d}", pressure);
